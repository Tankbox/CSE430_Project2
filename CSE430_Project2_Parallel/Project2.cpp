...
void debugger(GENI myGeni, Request myRequest)
{
	cout << "\nCurrently processing request from " + to_string(myRequest.source) + " to " + to_string(myRequest.destination) << endl;
	...
	for (int i = 0; i < myGeni.vertices.size(); ++i)
		cout << "Node " + to_string(myGeni.getName(i)) + ":\n\tVMs:\t" + 
			to_string(myGeni.vertices.at(i).VM) + "\n\tFTEs\t" + to_string(myGeni.vertices.at(i).FTE) << endl;
	...
	for (int i = 0; i < myGeni.connections.size(); ++i)
		cout << "Connection from " + to_string(myGeni.connections.at(i).nodes[0]->getName()) + 
			...
			to_string(myGeni.connections.at(i).bandwidth) << endl;

	cout << "\n\n";
}
void debugger(GENI myGeni)
{
	for (int i = 0; i < myGeni.vertices.size(); ++i)
	{
		cout << "\nNode " + to_string(myGeni.vertices.at(i).name) + " has degree " +
